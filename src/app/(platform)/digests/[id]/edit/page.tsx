...
import { BLOCK_TYPE_METADATA, getDefaultBlockConfig, getDigestById } from "~mocks/digest-data";
...
const EditDigestPage: React.FC = () => {
  ...
  const [recurrence, setRecurrence] = useState<RecurrenceType>(digest?.recurrence ?? "weekly");
  const [deliveryMethod, setDeliveryMethod] = useState<DeliveryMethod>(digest?.deliveryMethod ?? "both");
  ...
  if (!digest) {
    return (
      <div className="flex items-center justify-center h-[calc(100vh-16rem)]">
        <Card className="p-8">
          <CardHeader>
            <CardTitle>Digest Not Found</CardTitle>
            <CardDescription>The digest you're trying to edit doesn't exist.</CardDescription>
          </CardHeader>
          <CardContent>
            <Button
              onClick={() => {
  ...
  const handleSave = () => {
    ...
    alert("Digest updated successfully! (This is a prototype)");
    router.push(`/digests/${digestId}`);
  };
  ...
  return (
    <div className="space-y-6 max-w-4xl">
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-3xl font-bold text-foreground">Edit Digest</h1>
          <p className="text-muted-foreground mt-1">Update your AI-generated recurring report configuration</p>
        </div>
      ...
      <Card>
        <CardHeader>
          ...
          <CardDescription>Update the digest name, schedule, and delivery preferences</CardDescription>
        </CardHeader>
        ...
        <CardContent className="space-y-4">
          ...
          {blocks.length === 0 ? (
            <div className="p-8 text-center border border-dashed border-border rounded-lg bg-muted/20">
              ...
              <p className="text-sm font-medium text-foreground">No blocks configured</p>
